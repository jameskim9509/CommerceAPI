...
> **① 이 0 인 것이 정상이다** — 소부하에서는 재전송(2%)이 전부 카트 소진으로 튕긴다(실측 `replays=44 2xx=0`).
> 재전송이 중복 주문으로 살아남는 비율은 4~9% 수준이라 10만건 런에서만 `①`>0 이 된다.
> **③ 도 0 이 정상** — hot 재고 1,000 대 hot 주문 160 이라 재고가 소진되지 않는다.
> **`processed_events` 열이 arm 판별의 핵심이다** — 이 값이 라벨과 어긋나면 이미지·소스 불일치이므로
> 이미지를 재빌드하고 다시 측정할 것 (arm 전환 시 빌드 실패를 알아채지 못하는 사고가 실재했다).
