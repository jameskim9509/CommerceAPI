...
--   - 이메일 접두사 ctrich 로 런 스코프를 잡는다 (verify-consistency.sh 와 동일 규칙).
--   - cleanup 은 자기 행(seller id=1, ^ctrich[0-9]+@qa\.test$)만 — 멱등 재주입 안전.
...
-- ---------------- cleanup (자기 행만, FK 순서) ----------------
DELETE FROM customer_roles           WHERE customer_id IN (SELECT id FROM (SELECT id FROM customer WHERE email REGEXP '^ctrich[0-9]+@qa\\.test$') AS t);
DELETE FROM customer_balance_history WHERE CUSTOMER_ID IN (SELECT id FROM (SELECT id FROM customer WHERE email REGEXP '^ctrich[0-9]+@qa\\.test$') AS t);
DELETE FROM customer                 WHERE email REGEXP '^ctrich[0-9]+@qa\\.test$';
...
SELECT id, 'ROLE_CUSTOMER' FROM customer WHERE email REGEXP '^ctrich[0-9]+@qa\\.test$';
...
SELECT 'balance_history_rows', COUNT(*) FROM customer_balance_history h
    JOIN customer c ON h.CUSTOMER_ID = c.id WHERE c.email REGEXP '^ctrich[0-9]+@qa\\.test$';
