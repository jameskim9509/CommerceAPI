...
원하는 장애(중복 결제·재고 충돌·결제 후 재고 실패)와 주변 장애를 한 부하에서 동시에 발화시킨 통합 검증 **방법**은 [ADR-008](./008-order-consistency-integration-scenario.md) 을 참조한다 — 측정 KPI 는 5개 방어를 무방어→방어로 토글한 **위반 총수 집계 `N→r`** 이며, "`@Version` 이 초과판매를 막았다"는 개별 귀속은 하지 않는다. 통합 하네스는 ADR-008 명세대로 재작성 예정이라 실측치는 아직 `⟨측정전⟩` 이다.
